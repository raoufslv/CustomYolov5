...
class ConvNextBlock(nn.Module):
    ...
    # demo: [64, 64, 1]  1 denotes the number of repeats
    def __init__(self, inputdim, dim, drop_path=0., layer_scale_init_value=1e-6, kersize=7):
        super().__init__()
        # 匹配yolov5配置文件加入outdim输出通道
        # self.flag = True if dim == outdim else False

        self.dwconv = nn.Conv2d(dim, dim, kernel_size=kersize,
                                padding=kersize // 2, groups=dim)  # depthwise conv
...
class ODConv2d_3rd(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        if not save_parameters or self.kernel_size[0] * self.kernel_size[1] > 1:
            self.fc_s = nn.Linear(
                hidden_dim, self.kernel_size[0] * self.kernel_size[1])
    ...
    def reset_parameters(self) -> None:
        fan_out = self.kernel_size[0] * self.kernel_size[1] * \
    ...
    def debug(self, input, context=None):
        ...
        output_size = [
            ((H, W)[i] + 2 * self.padding[i] - self.dilation[i] *
        ...
        # B, groups, c_out // groups, c_in // groups * k * k
        weight = weight.view(
            B, self.groups, self.out_channels // self.groups, -1)

        unfold = nn.functional.unfold(
            input, self.kernel_size, self.dilation, self.padding, self.stride)  # B, c_in * k * k, H_out * W_out
        # B, groups, c_in // groups * k * k, H_out * W_out
...
class ODConv_3rd(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, kerNums=1, g=1, p=None, act=True):
        super().__init__()
        self.conv = ODConv2d_3rd(
            c1, c2, k, s, autopad(k, p), groups=g, K=kerNums)
        self.bn = nn.BatchNorm2d(c2)
...
class SENetC3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.m = nn.Sequential(
            *[SENetBottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])
